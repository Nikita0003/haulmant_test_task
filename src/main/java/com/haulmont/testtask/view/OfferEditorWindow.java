...
public class OfferEditorWindow extends Window {
    ...
    public OfferEditorWindow(Offer offer, MainUI.OPTIONS options)
    {
        ...
        if (offer.getCountPayment() <= 0) makingPaymentButton.setEnabled(false);
        ...
        //валидация полей
        binder.forField(sumTextField).withValidator(s -> s.matches("^[0-9]*[.,]?[0-9]+$"),
        ...
        okButton.addClickListener(clickEvent -> {
            try{
                ...
                double tempSum = offer.getTotalSum() * (1 + offer.getCredit().getPercent()/ 100) / 60; // общая сумма с процентами разделенная на количество выплат (принято условно так как не оговорено в задании)
                double tempBodySum = offer.getTotalSum() / 60;
                ...
                if (offer.getTotalSum() > offer.getCredit().getLimit())
                {
                    ...
                    if (options == MainUI.OPTIONS.ADD) Controller.addOffer(offer);
                    if (options == MainUI.OPTIONS.UPDATE) Controller.updateOffer(offer);
